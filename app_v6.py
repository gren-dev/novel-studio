...
def project_data() -> dict:
    ...
    data["app_version"] = "novel-studio-project-v6-publishing-target"
    return data
...
def render_market_review_controls(location_key: str, compact: bool = False, chapter_idx: int | None = None) -> None:
...
with st.sidebar:
    ...
    with st.expander("💾 Save / Load checkpoint", expanded=False):
        ...
        if uploaded_checkpoint is not None:
            try:
                data = json.loads(uploaded_checkpoint.read().decode("utf-8"))
                load_project(data)
                saved_stage = int(data.get("stage", data.get("checkpoint_stage", st.session_state.stage)))
                if 1 <= saved_stage <= len(STAGES):
                    st.session_state.stage = saved_stage
                st.success("Checkpoint loaded. Resuming from saved phase.")
                st.rerun()
            except Exception as exc:
...
Audience channel: {st.session_state.audience_channel}
Primary genre: {st.session_state.genre}
Multi-genre tags: {', '.join(st.session_state.genre_tags or [])}
Custom tags: {st.session_state.custom_genre_tags or 'None'}
...
# =============================================================================
    ...
    render_market_review_controls("market_stage", compact=False, chapter_idx=None)
