...
<style>
    ...
    .diff-box { border: 1px solid rgba(128,128,128,.25); border-radius: 12px; padding: 12px; margin: 8px 0; white-space: pre-wrap; line-height: 1.75; font-size: 0.95rem; max-height: 560px; overflow-y: auto; background: rgba(128,128,128,.035); }
    .diff-add { background: rgba(29,158,117,.18); border-bottom: 1px solid rgba(29,158,117,.55); padding: 0 2px; border-radius: 3px; }
    .diff-del { background: rgba(220,80,70,.16); text-decoration: line-through; color: #9b2f2a; padding: 0 2px; border-radius: 3px; }
    .diff-legend { font-size: 0.86rem; color: #777; margin-bottom: 0.35rem; }
...
def generate_text(system: str, user: str, placeholder, max_tokens: int = 3000, cleaner=None, task_name: str = "AI generation") -> str:
    """Call AI with generation locks and raw-output backup.
    ...
    session_state before rendering it in the placeholder. It also recovers from
    stale AI locks caused by a browser refresh, network drop, or interrupted run.
    """
    if st.session_state.get("ai_running", False):
        if clear_stale_ai_lock_if_needed():
            st.info("Recovered a stale AI lock from an interrupted generation. Starting this request now.")
        else:
            age = ai_lock_age_seconds()
            age_note = f" Running for about {int(age)}s." if age is not None else ""
            st.warning("AI is already generating. Please wait until the current task finishes." + age_note)
            return ""
...
PROJECT_FIELDS = [
    ...
    "chapter_versions", "pending_revisions", "pending_revision_sources", "pending_revision_meta", "local_refinements", "full_draft", "logic_report", "format_report", "polished_draft",
...
DEFAULTS = {
    ...
    "chapter_versions": {},
    "pending_revisions": {},
    "pending_revision_sources": {},
    "pending_revision_meta": {},
    "local_refinements": {},
    "full_draft": "",
    "logic_report": "",
    "format_report": "",
...
AI_LOCK_TIMEOUT_SECONDS = int(os.getenv("AI_LOCK_TIMEOUT_SECONDS", "600"))


def ai_lock_age_seconds() -> float | None:
    """Return current AI lock age in seconds, or None if unknown."""
    started = st.session_state.get("ai_started_at")
    if not started:
        return None
    try:
...
def reset_ai_lock(reason: str = "manual reset") -> None:
    """Clear a stuck AI lock without touching generated project content."""
    previous_task = st.session_state.get("ai_task_name", "")
    st.session_state.ai_running = False
    st.session_state.ai_task_name = ""
    st.session_state.ai_started_at = ""
    st.session_state["_last_action"] = f"AI lock reset ({reason}). Previous task: {previous_task or 'unknown'}."


def clear_stale_ai_lock_if_needed(max_age_seconds: int | None = None) -> bool:
    """Automatically clear an AI lock that has outlived the safe timeout."""
    if not st.session_state.get("ai_running", False):
        return False
    threshold = int(max_age_seconds or AI_LOCK_TIMEOUT_SECONDS)
    age = ai_lock_age_seconds()
    if age is None:
        reset_ai_lock("missing start timestamp")
        return True
    if age > threshold:
        reset_ai_lock(f"stale for {int(age)}s")
        return True
    return False


# Recover if the browser was refreshed while a previous AI call was mid-flight.
clear_stale_ai_lock_if_needed()
...
def set_story_bible_value(text: str) -> None:
    """Persist Story Bible text safely across Streamlit reruns.

    The visible editor uses key=story_bible_editor, while the canonical
    project field is story_bible. When an AI button finishes after the editor
    widget has already been created, we must not directly mutate the widget key
    in the same run. Instead, store a pending sync and apply it at the start of
    the next run before the widget is instantiated.
    """
    value = text or ""
    st.session_state.story_bible = value
    st.session_state["_pending_story_bible_editor_sync"] = value


def set_outline_value(text: str) -> list[dict]:
    """Persist outline text and parsed chapters together.

    This prevents the mobile flow from showing a freshly generated outline in
    the temporary output placeholder and then returning to an empty Stage 3 on
    rerun because only the placeholder had content.
    """
    value = text or ""
    st.session_state.outline = value
    chapters = parse_chapters(value) if value.strip() else []
    st.session_state.chapters = chapters
    st.session_state["_last_outline_persisted_at"] = datetime.now().isoformat(timespec="seconds")
    return chapters


def apply_pending_widget_syncs() -> None:
    """Apply deferred widget-key updates before stage widgets are created."""
    if "_pending_story_bible_editor_sync" in st.session_state:
        value = st.session_state.pop("_pending_story_bible_editor_sync")
        st.session_state.story_bible_editor = value
        st.session_state["_story_bible_editor_source"] = value


def _diff_chunks(original: str, proposed: str, max_chars: int = 50000) -> tuple[str, bool]:
    """Return safe HTML highlighting insertions/deletions between two drafts."""
    original = original or ""
    proposed = proposed or ""
    truncated = False
    if len(original) + len(proposed) > max_chars:
        half = max_chars // 2
        original = original[:half]
        proposed = proposed[:half]
        truncated = True
    matcher = difflib.SequenceMatcher(None, original, proposed, autojunk=False)
    parts: list[str] = []
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        old = html.escape(original[i1:i2])
        new = html.escape(proposed[j1:j2])
        if tag == "equal":
            parts.append(new)
        elif tag == "insert":
            parts.append(f'<span class="diff-add">{new}</span>')
        elif tag == "delete":
            parts.append(f'<span class="diff-del">{old}</span>')
        elif tag == "replace":
            if old:
                parts.append(f'<span class="diff-del">{old}</span>')
            if new:
...
def render_change_highlight(original: str, proposed: str, label: str = "Highlight changes / 高亮修改") -> None:
    """Show an inline diff for a revision proposal without changing saved text."""
    original = original or ""
    proposed = proposed or ""
    if not original.strip() or not proposed.strip():
        st.info("Need both original and proposal to highlight changes.")
        return
    html_diff, truncated = _diff_chunks(original, proposed)
    st.markdown(f"**{label}**")
    st.markdown('<div class="diff-legend"><span class="diff-add">绿色</span> = 新增/改后内容； <span class="diff-del">红色删除线</span> = 原文被删除或替换。</div>', unsafe_allow_html=True)
    if truncated:
        st.warning("Text is long, so the diff preview is truncated. The full original/proposal text below is unchanged.")
...
def _paragraph_units(text: str) -> list[str]:
    """Split text into paragraph-like units while preserving separators."""
    text = text or ""
    if not text:
        return []
    parts = re.split(r"(\n\s*\n+)", text)
...
def build_change_blocks(original: str, proposed: str) -> list[dict]:
    """Build paragraph-level change blocks for selective accept/reject review."""
    old_units = _paragraph_units(original or "")
    new_units = _paragraph_units(proposed or "")
    old_norm = [_normalize_unit(x) for x in old_units]
    new_norm = [_normalize_unit(x) for x in new_units]
    matcher = difflib.SequenceMatcher(None, old_norm, new_norm, autojunk=False)
    blocks: list[dict] = []
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        old = "".join(old_units[i1:i2])
        new = "".join(new_units[j1:j2])
        if tag == "equal":
            blocks.append({"tag": "equal", "original": old, "proposed": new})
        else:
            blocks.append({"tag": tag, "original": old, "proposed": new})
    return blocks


def render_block_review_controls(original: str, proposed: str, base_key: str, preview_height: int = 260) -> tuple[str, int]:
    """Render paragraph-level accept/reject controls and return the reviewed text."""
    original = original or ""
    proposed = proposed or ""
    blocks = build_change_blocks(original, proposed)
    change_count = sum(1 for b in blocks if b.get("tag") != "equal")
    if change_count == 0:
        st.info("No paragraph-level changes found.")
        return proposed, 0

    st.caption("逐段审阅：每处修改可以单独接受，或保留原文。未变化段落会自动保留。")
    final_parts: list[str] = []
    change_no = 0
    for block_i, block in enumerate(blocks):
        tag = block.get("tag")
        old = block.get("original", "") or ""
        new = block.get("proposed", "") or ""
        if tag == "equal":
            final_parts.append(new)
            continue
        change_no += 1
        title_map = {
            "replace": "修改",
        ...
        with st.expander(f"Change {change_no}: {title_map.get(tag, tag)}", expanded=(change_no <= 2)):
            if old.strip():
                st.markdown("**原文 / Original**")
                st.text_area("Original block", value=old.strip(), height=140, key=f"{base_key}_old_{block_i}", disabled=True, label_visibility="collapsed")
            if new.strip():
                st.markdown("**提案 / Proposed**")
                st.text_area("Proposed block", value=new.strip(), height=140, key=f"{base_key}_new_{block_i}", disabled=True, label_visibility="collapsed")
            if tag == "insert":
                options = ["接受新增", "不要新增"]
            elif tag == "delete":
                options = ["接受删除", "保留原文"]
                ...
                options = ["接受修改", "保留原文"]
            choice = st.radio("Decision", options, horizontal=True, key=f"{base_key}_choice_{block_i}", label_visibility="collapsed")
            if choice in ("接受新增", "接受修改"):
                final_parts.append(new)
            elif choice == "接受删除":
                final_parts.append("")
            else:
                final_parts.append(old)

    reviewed_text = "".join(final_parts).strip()
    with st.expander("Reviewed result preview / 已审阅结果预览", expanded=False):
        st.text_area("Reviewed result", value=reviewed_text, height=preview_height, key=f"{base_key}_reviewed_preview", disabled=True, label_visibility="collapsed")
    return reviewed_text, change_count
...
def normalize_loaded_project(data: dict) -> dict:
    ...
    for dict_key in ["chapter_beats", "chapter_drafts", "chapter_versions", "pending_revisions", "pending_revision_sources", "pending_revision_meta", "local_refinements", "quality_reports", "market_reviews", "revision_notes"]:
...
def project_data() -> dict:
    ...
    data["app_version"] = "novel-studio-project-v25-5-mobile-story-outline-persist"
    return data
...
""",
    ...
    if refinement:
        ...
        with st.expander("Highlight changes / 高亮修改", expanded=True):
            render_change_highlight(refinement.get("source", ""), refinement.get("result", ""), "Local refinement diff / 局部精修差异")
        a, b, c = st.columns(3)
        with a:
        ...
        if proposals:
            ...
            for idx in sorted(proposals.keys()):
                ...
                with st.expander(f"{label} — {item.get('mode', '')}"):
                    ...
                    with st.expander("Highlight changes / 高亮修改", expanded=True):
                        render_change_highlight(item.get("original", ""), item.get("proposal", ""), "Batch revision diff / 批量修改差异")
                    c1, c2 = st.columns(2)
                    with c1:
...
def current_view_mode() -> str:
    # v25.3 removes the confusing "Auto" label. Streamlit cannot reliably
    # detect phone width without a custom component, so mode is explicit.
    mode = st.session_state.get("view_mode", "Desktop")
    if mode == "Auto":
        mode = "Desktop"
...
def render_mobile_top_nav() -> None:
    ...
    st.caption("手机端使用独立写作路径；复杂设置仍可切回 Desktop。")
    nav_items = mobile_stage_map()
    cols = st.columns(len(nav_items))
    locked = bool(st.session_state.get("ai_running"))
    for col, (label, target_stage) in zip(cols, nav_items):
        active = target_stage == int(st.session_state.get("stage", 1) or 1)
        ...
        with col:
            if st.button(
                ...
                use_container_width=True,
                disabled=locked,
                key=f"mobile_nav_btn_{target_stage}",
            ):
...
def render_mobile_dashboard() -> None:
    ...
    locked = bool(st.session_state.get("ai_running"))
    with c1:
        if st.button("✍️ Write", use_container_width=True, disabled=locked, key="mobile_dash_write"):
            st.session_state.stage = 5
            st.rerun()
    with c2:
        if st.button("💾 Save", use_container_width=True, disabled=locked, key="mobile_dash_save"):
            try:
                ...
                st.error(f"Cloud save failed: {exc}")
    with c3:
        if st.button("📥 Export", use_container_width=True, disabled=locked, key="mobile_dash_export"):
            st.session_state.stage = 10
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)


def render_mobile_write_page() -> None:
    """Phone-first writing screen. This replaces the full desktop Draft Writer in Mobile mode."""
    st.markdown('<div class="stage-header">✍️ Mobile Write / 手机续写</div>', unsafe_allow_html=True)
    st.caption("手机端只保留高频写作动作：选章、看正文、续写、改写、保存。复杂批量工具折叠在下面。")

    if not st.session_state.get("chapters"):
        st.warning("还没有识别到章节。请先在 Outline 生成或重新识别章节。")
        c1, c2 = st.columns(2)
        with c1:
            if st.button("去 Outline", use_container_width=True, key="mobile_no_chapters_outline"):
                st.session_state.stage = 3
                st.rerun()
        with c2:
            if st.button("保存当前项目", use_container_width=True, key="mobile_no_chapters_save"):
                cloud_save_project_guarded(st.session_state.get("auth_user", "user"), default_cloud_project_name(), force=True, save_reason="mobile save without chapters")
                st.success("Saved to cloud.")
        return

    chapters = st.session_state.chapters
    max_idx = len(chapters) - 1
    default_idx = int(st.session_state.get("mobile_selected_chapter_idx", 0) or 0)
    default_idx = min(max(default_idx, 0), max_idx)
    labels = [f"{chapter_heading(ch)} {'✓' if i in st.session_state.chapter_drafts else ''}" for i, ch in enumerate(chapters)]
    idx = st.selectbox(
        "当前章节",
        range(len(labels)),
        index=default_idx,
        format_func=lambda i: labels[i],
        key="mobile_write_chapter_select",
    )
    st.session_state.mobile_selected_chapter_idx = idx
    ch = chapters[idx]

    prev_col, save_col, next_col = st.columns(3)
    locked = bool(st.session_state.get("ai_running"))
    with prev_col:
        if st.button("← 上一章", use_container_width=True, disabled=locked or idx <= 0, key="mobile_prev_chapter"):
            st.session_state.mobile_selected_chapter_idx = idx - 1
            st.rerun()
    with save_col:
        if st.button("💾 保存", use_container_width=True, disabled=locked, key="mobile_write_save"):
            cloud_save_project_guarded(st.session_state.get("auth_user", "user"), default_cloud_project_name(), force=True, save_reason=f"mobile save {chapter_heading(ch)}")
            st.success("Saved to cloud.")
    with next_col:
        if st.button("下一章 →", use_container_width=True, disabled=locked or idx >= max_idx, key="mobile_next_chapter"):
            st.session_state.mobile_selected_chapter_idx = idx + 1
            st.rerun()

    with st.expander("本章摘要 / Outline summary", expanded=False):
        st.write(ch.get("summary", "") or "暂无摘要")
        beat = st.session_state.chapter_beats.get(idx, "")
        if beat:
            st.markdown("**Beat Sheet**")
            st.write(beat)

    length_target = st.selectbox("生成长度", ["Short scene", "Normal chapter", "Long chapter"], index=1, key="mobile_length_target")
    length_rule = {
        "Short scene": "700-1000 English words or 1000-1800 Chinese characters",
        "Normal chapter": "1000-1600 English words or 1800-3000 Chinese characters",
        "Long chapter": "1600-2400 English words or 3000-4500 Chinese characters",
    ...
    if not draft:
        st.info("这一章还没有正文。")
        if st.button(f"生成 {chapter_heading(ch)} ✨", type="primary", use_container_width=True, disabled=locked, key="mobile_generate_draft"):
            ...
            if result:
                ...
                rebuild_full_draft()
                mark_dirty()
                cloud_auto_save(f"mobile draft {chapter_heading(ch)}")
                st.success("Chapter drafted.")
                st.rerun()
    else:
        st.subheader("Current draft / 当前正文")
        draft_key = f"mobile_draft_editor_{idx}"
        draft_hash_key = f"mobile_draft_hash_{idx}"
        current_hash = hashlib.md5(draft.encode("utf-8")).hexdigest() if draft else ""
        if draft_key not in st.session_state or st.session_state.get(draft_hash_key) != current_hash:
            st.session_state[draft_key] = draft
            st.session_state[draft_hash_key] = current_hash
        edited = st.text_area("正文", key=draft_key, height=420, label_visibility="collapsed")
        if edited != draft:
            st.session_state.chapter_drafts[idx] = edited
            st.session_state[draft_hash_key] = hashlib.md5(edited.encode("utf-8")).hexdigest() if edited else ""
            rebuild_full_draft()
            mark_dirty()
            st.info("已保存到当前 session。点上方保存写入云端。")

        action_col1, action_col2 = st.columns(2)
        with action_col1:
            if st.button("继续写本章 ✨", type="primary", use_container_width=True, disabled=locked, key="mobile_continue_chapter"):
                current = st.session_state.chapter_drafts.get(idx, "")
...
Current draft ending:
{current[-2200:]}

Continue this chapter from the exact ending above. Keep continuity. Return only the continuation prose, not a recap.
"""
                with st.spinner("Continuing chapter..."):
                    placeholder = st.empty()
                    addition = generate_text(
                        system="You are a skilled fiction writer. Continue the scene with concrete action, dialogue, and continuity.",
                        user=prompt,
                        placeholder=placeholder,
                        max_tokens=2500,
                        cleaner=clean_chapter_draft_output,
                ...
                if addition:
                    push_version(idx, current, "Before mobile continuation")
                    st.session_state.chapter_drafts[idx] = (current.rstrip() + "\n\n" + addition.strip()).strip()
                    rebuild_full_draft()
                    mark_dirty()
                    cloud_auto_save(f"mobile continue {chapter_heading(ch)}")
                    st.success("Continuation appended.")
                    st.rerun()
        with action_col2:
            if st.button("改写本章 🔁", use_container_width=True, disabled=locked, key="mobile_revise_chapter"):
                instruction = "Rewrite this chapter to reduce AI-like phrasing, keep the plot, strengthen scene detail and dialogue, and preserve continuity."
                with st.spinner("Creating revision proposal..."):
                    placeholder = st.empty()
                    result = generate_text(
                        system="You are a professional fiction editor. Return only the revised chapter text.",
                        user=f"""Writing context:
{writing_context()}
...
""",
                ...
                if result:
                    ...
                    st.session_state.pending_revision_sources[idx] = st.session_state.chapter_drafts.get(idx, "")
                    st.session_state.pending_revision_meta[idx] = {"type": "full_chapter", "label": "Mobile chapter revision"}
                    cloud_auto_save(f"mobile revision proposal {chapter_heading(ch)}")
                    st.success("Revision proposal created.")
                    st.rerun()

        if st.session_state.pending_revisions.get(idx):
            st.subheader("Revision proposal / 修改提案")
            with st.expander("Highlight changes / 高亮修改", expanded=True):
                render_change_highlight(st.session_state.pending_revision_sources.get(idx, st.session_state.chapter_drafts.get(idx, "")), st.session_state.pending_revisions[idx], "Mobile revision diff / 手机改写差异")
            st.text_area("Proposed revision", value=st.session_state.pending_revisions[idx], height=360, key=f"mobile_prop_{idx}")
            source_text = st.session_state.pending_revision_sources.get(idx, st.session_state.chapter_drafts.get(idx, ""))
            proposal_text = st.session_state.pending_revisions[idx]
            review_hash = hashlib.md5((source_text + "\n---\n" + proposal_text).encode("utf-8")).hexdigest()[:10]
            with st.expander("Review changes by block / 分段接受或拒绝", expanded=True):
                reviewed_text, change_count = render_block_review_controls(source_text, proposal_text, f"mobile_block_review_{idx}_{review_hash}", preview_height=300)
                if st.button("应用已审阅版本", type="primary", use_container_width=True, key=f"mobile_apply_reviewed_{idx}_{review_hash}"):
                    push_version(idx, st.session_state.chapter_drafts[idx], "Before applied mobile reviewed revision")
                    st.session_state.chapter_drafts[idx] = reviewed_text
                    st.session_state.pending_revisions.pop(idx, None)
                    st.session_state.pending_revision_sources.pop(idx, None)
                    st.session_state.pending_revision_meta.pop(idx, None)
                    rebuild_full_draft()
                    mark_dirty()
                    cloud_auto_save(f"mobile applied reviewed revision {chapter_heading(ch)}")
                    st.rerun()
            a, b = st.columns(2)
            with a:
                if st.button("接受提案", type="primary", use_container_width=True, key="mobile_accept_revision"):
                    push_version(idx, st.session_state.chapter_drafts[idx], "Before accepted mobile revision")
                    st.session_state.chapter_drafts[idx] = st.session_state.pending_revisions.pop(idx)
                    st.session_state.pending_revision_sources.pop(idx, None)
                    st.session_state.pending_revision_meta.pop(idx, None)
                    rebuild_full_draft()
                    mark_dirty()
                    cloud_auto_save(f"mobile accepted revision {chapter_heading(ch)}")
                    st.rerun()
            with b:
                if st.button("丢弃提案", use_container_width=True, key="mobile_discard_revision"):
                    st.session_state.pending_revisions.pop(idx, None)
                    st.session_state.pending_revision_sources.pop(idx, None)
                    st.session_state.pending_revision_meta.pop(idx, None)
                    st.rerun()

    with st.expander("Advanced tools / 高级工具", expanded=False):
        render_quick_reference(idx)
        render_generate_full_text_panel("mobile_write")
        render_batch_revision_panel("mobile_write")
        if st.session_state.chapter_drafts.get(idx):
            render_local_refinement_panel(idx, st.session_state.chapter_drafts[idx], f"mobile_draft_{idx}")
            render_market_review_controls(f"mobile_quick_{idx}", compact=True, chapter_idx=idx)
...
with st.sidebar:
    ...
    st.radio(
        ...
        key="view_mode",
        horizontal=True,
        help="Mobile mode uses a phone-first writing screen. Desktop keeps the full workspace.",
    )
    gemini_key = get_gemini_api_key()
    bigmodel_key = get_bigmodel_api_key()
    ...
    with st.expander("🧪 AI connection check / AI 状态恢复", expanded=False):
        st.caption("Use this if Generate buttons seem to do nothing or the app says AI is already generating.")
        if st.session_state.get("ai_running"):
            age = ai_lock_age_seconds()
            age_msg = f" about {int(age)}s" if age is not None else " unknown time"
            st.warning(f"AI task is marked running for{age_msg}: {st.session_state.get('ai_task_name') or 'unknown task'}")
            if st.button("Force unlock AI state / 强制解除 AI 卡住状态", use_container_width=True, key="force_unlock_ai_state_v25_9"):
                reset_ai_lock("manual sidebar button")
                st.success("AI lock cleared. You can click Generate again.")
                st.rerun()
...
# In Mobile mode, the Write page is a dedicated phone-first route.
# Do not render the full desktop Draft Writer underneath it.
if is_mobile_mode() and int(st.session_state.get("stage", 1) or 1) == 5:
    render_mobile_write_page()
    st.stop()
...
# =============================================================================
    ...
    # Use a separate widget key so AI buttons can update st.session_state.story_bible
    # after the text_area has been instantiated in the same Streamlit run.
    # Directly using key="story_bible" causes StreamlitAPIException when generation
    # tries to assign st.session_state.story_bible later in this block.
    def _sync_story_bible_editor():
        st.session_state.story_bible = st.session_state.get("story_bible_editor", "")
        st.session_state["_story_bible_editor_source"] = st.session_state.story_bible
        mark_dirty()

    current_story_bible = st.session_state.get("story_bible", "")
    if (
        "story_bible_editor" not in st.session_state
        or st.session_state.get("_story_bible_editor_source") != current_story_bible
    ):
        st.session_state.story_bible_editor = current_story_bible
        st.session_state["_story_bible_editor_source"] = current_story_bible

    st.text_area(
        "Full Story Bible / 总设定集",
        key="story_bible_editor",
        height=260,
        placeholder="核心设定、人物关系、伏笔、禁忌、世界规则、写作风格规则……",
        on_change=_sync_story_bible_editor,
    )
    ...
    with st.expander("🧯 Story Bible recovery / 补全与恢复", expanded=story_bible_is_incomplete(st.session_state.get("story_bible", ""))):
        ...
        if st.session_state.get("_last_story_bible_raw"):
            if st.button("Restore last raw AI output / 恢复最近原始输出", use_container_width=True):
                set_story_bible_value(st.session_state.get("_last_story_bible_raw", ""))
                mark_dirty()
                st.rerun()
...
# =============================================================================
    ...
    if st.session_state.get("ai_running"):
        age = ai_lock_age_seconds()
        if age is not None:
            st.info(f"AI task still marked running for about {int(age)}s. If the page is stuck, open AI connection check in the sidebar and use Force unlock.")
    ...
    if local_clicked:
        ...
        set_outline_value(build_local_outline_from_material())
...
""",
            ...
            if result:
                st.session_state.pending_revisions[idx] = result
                st.session_state.pending_revision_sources[idx] = st.session_state.chapter_drafts.get(idx, "")
                st.session_state.pending_revision_meta[idx] = {"type": "full_chapter", "label": "Draft Writer revision"}
        ...
        if st.session_state.pending_revisions.get(idx):
            ...
            with st.expander("Highlight changes / 高亮修改", expanded=True):
                render_change_highlight(st.session_state.pending_revision_sources.get(idx, st.session_state.chapter_drafts.get(idx, "")), st.session_state.pending_revisions[idx], "Revision diff / 改写差异")
            left, right = st.columns(2)
            with left:
            ...
            review_hash = hashlib.md5((source_text + "\n---\n" + proposal_text).encode("utf-8")).hexdigest()[:10]
            with st.expander("Review changes by block / 分段接受或拒绝", expanded=True):
                reviewed_text, change_count = render_block_review_controls(source_text, proposal_text, f"draft_block_review_{idx}_{review_hash}", preview_height=320)
                if st.button("Apply reviewed version", type="primary", key=f"draft_apply_reviewed_{idx}_{review_hash}"):
                    push_version(idx, st.session_state.chapter_drafts[idx], "Before applied reviewed Draft Writer revision")
                    st.session_state.chapter_drafts[idx] = reviewed_text
                    st.session_state.pending_revisions.pop(idx, None)
                    st.session_state.pending_revision_sources.pop(idx, None)
                    st.session_state.pending_revision_meta.pop(idx, None)
                    rebuild_full_draft()
                    mark_dirty()
                    cloud_auto_save(f"applied reviewed revision {chapter_heading(ch)}")
                    st.success("Reviewed version applied.")
                    st.rerun()
            a, b, c = st.columns(3)
            with a:
...
""",
            ...
            if result:
                st.session_state.pending_revisions[idx] = result
                st.session_state.pending_revision_sources[idx] = source
                st.session_state.pending_revision_meta[idx] = {"type": "selected_passage", "label": action}
                cloud_auto_save(f"revision studio proposal {chapter_heading(ch)}")
                st.success("Output created. Review the highlighted changes below before copying, appending, or replacing text.")
                with st.expander("Highlight changes / 高亮修改", expanded=True):
                    render_change_highlight(source, result, "Revision Studio diff / 修改工作台差异")
                st.text_area("Generated proposal", value=result, height=360, key=f"revision_generated_{idx}")
...
""",
    ...
    if st.session_state.pending_revisions.get(idx):
        st.divider()
        st.subheader("Pending proposal / 待处理提案")
        source_for_diff = st.session_state.pending_revision_sources.get(idx, draft)
        meta = st.session_state.pending_revision_meta.get(idx, {}) or {}
        st.caption(f"Source: {meta.get('label', 'revision proposal')}")
        with st.expander("Highlight changes / 高亮修改", expanded=True):
            render_change_highlight(source_for_diff, st.session_state.pending_revisions[idx], "Desktop revision diff / 桌面端改写差异")
        left, right = st.columns(2)
        with left:
            st.markdown("**Original / 原文**")
            st.text_area("Original for comparison", value=source_for_diff, height=320, key=f"desktop_stage6_orig_{idx}")
        with right:
            st.markdown("**Proposal / 提案**")
            st.text_area("Proposal for comparison", value=st.session_state.pending_revisions[idx], height=320, key=f"desktop_stage6_prop_{idx}")

        source_text = source_for_diff
        proposal_text = st.session_state.pending_revisions[idx]
        review_hash = hashlib.md5((source_text + "\n---\n" + proposal_text).encode("utf-8")).hexdigest()[:10]
        with st.expander("Review changes by block / 分段接受或拒绝", expanded=True):
            reviewed_text, change_count = render_block_review_controls(source_text, proposal_text, f"stage6_block_review_{idx}_{review_hash}", preview_height=320)
            if st.button("Apply reviewed version", type="primary", key=f"stage6_apply_reviewed_{idx}_{review_hash}"):
                push_version(idx, st.session_state.chapter_drafts[idx], "Before applied reviewed Revision Studio proposal")
                st.session_state.chapter_drafts[idx] = reviewed_text
                st.session_state.pending_revisions.pop(idx, None)
                st.session_state.pending_revision_sources.pop(idx, None)
                st.session_state.pending_revision_meta.pop(idx, None)
                rebuild_full_draft()
                mark_dirty()
                cloud_auto_save(f"applied reviewed revision studio proposal {chapter_heading(ch)}")
                st.success("Reviewed version applied.")
                st.rerun()

        c1, c2, c3, c4 = st.columns(4)
        with c1:
            if st.button("Accept as full chapter", type="primary", key=f"stage6_accept_full_{idx}"):
                push_version(idx, st.session_state.chapter_drafts[idx], "Before accepted Revision Studio proposal")
                st.session_state.chapter_drafts[idx] = st.session_state.pending_revisions.pop(idx)
                st.session_state.pending_revision_sources.pop(idx, None)
                st.session_state.pending_revision_meta.pop(idx, None)
                rebuild_full_draft()
                cloud_auto_save(f"accepted revision studio proposal {chapter_heading(ch)}")
                st.success("Proposal accepted as the full chapter.")
                st.rerun()
        with c2:
            if st.button("Append to chapter", key=f"stage6_append_{idx}"):
                push_version(idx, st.session_state.chapter_drafts[idx], "Before appended Revision Studio proposal")
                addition = st.session_state.pending_revisions.pop(idx)
                st.session_state.chapter_drafts[idx] = (st.session_state.chapter_drafts.get(idx, "").rstrip() + "\n\n" + addition.strip()).strip()
                st.session_state.pending_revision_sources.pop(idx, None)
                st.session_state.pending_revision_meta.pop(idx, None)
                rebuild_full_draft()
                cloud_auto_save(f"appended revision studio proposal {chapter_heading(ch)}")
                st.success("Proposal appended to the chapter.")
                st.rerun()
        with c3:
            if st.button("Save proposal version", key=f"stage6_save_prop_{idx}"):
                push_version(idx, st.session_state.pending_revisions[idx], "Saved Revision Studio proposal")
                cloud_auto_save(f"saved revision studio proposal {chapter_heading(ch)}")
                st.success("Proposal saved in version history.")
        with c4:
            if st.button("Discard proposal", key=f"stage6_discard_{idx}"):
                st.session_state.pending_revisions.pop(idx, None)
                st.session_state.pending_revision_sources.pop(idx, None)
                st.session_state.pending_revision_meta.pop(idx, None)
                st.rerun()
